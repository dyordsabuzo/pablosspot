...
class AccessKey(object):
    ...
    def create_access_key(self):
        localstack_hostname = os.environ['LOCALSTACK_HOSTNAME'] if 'LOCALSTACK_HOSTNAME' in os.environ else None
        endpoint_url = f'http://{localstack_hostname}:4566' if localstack_hostname != None else None

        iam = boto3.client('iam', endpoint_url=endpoint_url)

        accesskeys = iam.list_access_keys(UserName=self.user)[
            'AccessKeyMetadata']

        if len(accesskeys) > 0:
            keys = [a['AccessKeyId']
                    for a in sorted(accesskeys, key=lambda x: x['CreateDate'])]
            if len(keys) > 1:
